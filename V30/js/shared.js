...
(function(){
  ...
  var NAV_LINKS = [
    ...
    { href: '/vitals.html', key: 'nav_vitals', label: 'Check Vitals', labelEs: 'Revisar Signos' },
    ...
    { href: '/grow.html', key: 'nav_grow', label: 'Grow', labelEs: 'Cultivar' },
    ...
    { divider: true },
    { href: '/contribute.html', key: 'nav_contribute', label: '🤲 Contribute', labelEs: '🤲 Contribuir' },
  ...
  /* ── Civic Loop (injected on every page) ── */
  function buildCivicLoop() {
    ...
    var style = document.createElement('style');
    style.textContent = [
      '.civic-loop-wrap{max-width:480px;margin:30px auto 10px;text-align:center;padding:0 10px}',
      '.civic-loop-wrap h2{font-family:var(--display);font-size:1.2rem;font-weight:600;color:var(--deep);margin-bottom:4px}',
      '.civic-loop-wrap .cl-sub{font-size:.75rem;color:var(--dim);margin-bottom:14px;line-height:1.5}',
      '.cl-ring{position:relative;width:100%;padding-bottom:105%}',
      '.cl-ring svg{position:absolute;top:0;left:0;width:100%;height:100%;pointer-events:none;z-index:0}',
      '.cl-node{position:absolute;z-index:1;display:flex;flex-direction:column;align-items:center;text-decoration:none;',
      '  width:86px;padding:10px 5px;border-radius:14px;background:var(--cream);border:2.5px solid var(--nc,var(--sage));',
      '  transition:transform .2s,box-shadow .2s}',
      '.cl-node:hover{transform:scale(1.08);box-shadow:0 4px 18px rgba(0,0,0,.1)}',
      '.cl-node.cl-here{border-width:3.5px;box-shadow:0 0 0 4px rgba(90,138,120,.2);transform:scale(1.06)}',
      '.cl-node.cl-here::after{content:"📍";position:absolute;top:-12px;right:-8px;font-size:.75rem}',
      '.cl-node .cli{font-size:1.2rem;margin-bottom:2px}',
      '.cl-node .cln{font-family:var(--display);font-size:.72rem;font-weight:600;color:var(--nc,var(--deep))}',
      '.cl-node .cld{font-size:.54rem;color:var(--dim);line-height:1.3;margin-top:1px}',
      '.cl-ret{margin-top:8px;padding:8px 14px;background:rgba(196,101,74,.06);border-radius:8px;border:1px solid rgba(196,101,74,.12);font-size:.7rem;color:var(--coral);line-height:1.5}',
      '#cln-0{top:0%;left:50%;transform:translateX(-50%)}#cln-0:hover,#cln-0.cl-here{transform:translateX(-50%) scale(1.08)}',
      '#cln-1{top:28%;right:0%}#cln-2{top:68%;right:8%}#cln-3{top:68%;left:8%}#cln-4{top:28%;left:0%}',
      '@media(max-width:400px){.cl-node{width:72px;padding:8px 4px;border-radius:10px}.cli{font-size:1rem}.cln{font-size:.64rem}.cld{font-size:.48rem}',
      '#cln-1{right:-2%}#cln-4{left:-2%}#cln-2{right:2%}#cln-3{left:2%}}',
      '@keyframes clPulse{0%,100%{opacity:.3;stroke-width:1.8}50%{opacity:.7;stroke-width:2.8}}'
    ].join('\n');
    document.head.appendChild(style);
    ...
    var labels = isEs
      ...
      : { title:'The Civic Loop', sub:'Every tool connects. Click any stage to go there.',
          ...
          d:['Food, health, emergency help','Know how systems work','Find and help neighbors','Hold leaders accountable','Track promises with evidence'],
    ...
    var icons = ['🔥','📖','🤝','⚖️','📋'];
    var colors = ['#c4654a','#b8963e','#5a8a78','#1a3c34','#5a8a78'];
    var pages = ['survive','understand','connect','govern','audit'];

    var nodesHtml = '';
    for (var i = 0; i < 5; i++) {
      nodesHtml += '<a href="' + hrefs[i] + '" class="cl-node" id="cln-' + i + '" data-pg="' + pages[i] + '" style="--nc:' + colors[i] + '">' +
        '<div class="cli">' + icons[i] + '</div>' +
        '<div class="cln">' + labels.n[i] + '</div>' +
        '<div class="cld">' + labels.d[i] + '</div></a>';
    }

    var wrap = document.createElement('div');
    wrap.className = 'civic-loop-wrap';
    wrap.innerHTML =
      '<h2>' + labels.title + '</h2>' +
      '<p class="cl-sub">' + labels.sub + '</p>' +
      '<div class="cl-ring" id="clRing">' +
        '<svg class="cl-svg" id="clSvg" viewBox="0 0 400 420" xmlns="http://www.w3.org/2000/svg">' +
          '<defs><marker id="clf" markerWidth="7" markerHeight="5" refX="7" refY="2.5" orient="auto"><path d="M0,0 L7,2.5 L0,5" fill="#5a8a78" opacity=".5"/></marker>' +
          '<marker id="clr" markerWidth="7" markerHeight="5" refX="7" refY="2.5" orient="auto"><path d="M0,0 L7,2.5 L0,5" fill="#c4654a" opacity=".6"/></marker></defs>' +
          '<path id="cla-01" stroke="#5a8a78" stroke-width="2" fill="none" marker-end="url(#clf)" opacity=".4" d="M0,0"/>' +
          '<path id="cla-12" stroke="#5a8a78" stroke-width="2" fill="none" marker-end="url(#clf)" opacity=".4" d="M0,0"/>' +
          '<path id="cla-23" stroke="#5a8a78" stroke-width="2" fill="none" marker-end="url(#clf)" opacity=".4" d="M0,0"/>' +
          '<path id="cla-34" stroke="#5a8a78" stroke-width="2" fill="none" marker-end="url(#clf)" opacity=".4" d="M0,0"/>' +
          '<path id="cla-40" stroke="#c4654a" stroke-width="2.5" fill="none" stroke-dasharray="6,4" marker-end="url(#clr)" opacity=".5" d="M0,0"/>' +
        '</svg>' +
        nodesHtml +
      '</div>' +
      '<div class="cl-ret">↩ ' + labels.ret + '</div>';

    // Insert before footer (or at end of .page)
    var footer = page.querySelector('.site-footer');
    if (footer) { page.insertBefore(wrap, footer); } else { page.appendChild(wrap); }

    // You-are-here
    var pg = location.pathname.replace(/.*\//,'').replace('.html','');
    var hereEl = wrap.querySelector('[data-pg="' + pg + '"]');
    if (hereEl) hereEl.classList.add('cl-here');

    // Draw arrows after layout
    setTimeout(function() {
      var ring = document.getElementById('clRing');
      if (!ring) return;
      var w = ring.offsetWidth, h = ring.offsetHeight;
      var ns = [];
      for (var j = 0; j < 5; j++) {
        var el = document.getElementById('cln-' + j);
        if (!el) return;
        ns.push({ x: el.offsetLeft + el.offsetWidth/2, y: el.offsetTop + el.offsetHeight/2, t: el.offsetTop, b: el.offsetTop + el.offsetHeight });
      }
      var pairs = [[0,1],[1,2],[2,3],[3,4]];
      pairs.forEach(function(p) {
        var a = ns[p[0]], b = ns[p[1]];
        var dx = b.x-a.x, dy = b.y-a.y, len = Math.sqrt(dx*dx+dy*dy);
        var ux = dx/len, uy = dy/len;
        var x1 = a.x+ux*48, y1 = a.y+uy*48, x2 = b.x-ux*48, y2 = b.y-uy*48;
        var mx = (x1+x2)/2+(-uy)*16, my = (y1+y2)/2+(ux)*16;
        var path = document.getElementById('cla-'+p[0]+p[1]);
        if (path) { path.setAttribute('d','M'+x1+','+y1+' Q'+mx+','+my+' '+x2+','+y2); path.style.animation = 'clPulse 3s ease-in-out '+(p[0]*0.5)+'s infinite'; }
      });
      // Return arc
      var ret = document.getElementById('cla-40');
      if (ret) {
        var a4 = ns[4], a0 = ns[0];
        ret.setAttribute('d','M'+a4.x+','+(a4.t-5)+' C'+(a4.x-60)+','+(h*0.95)+' '+(a0.x+60)+','+(h*0.95)+' '+a0.x+','+(a0.t-5));
        ret.style.animation = 'clPulse 4s ease-in-out 2s infinite';
      }
    }, 150);
  }
  ...
  var CROSS_DOMAIN = {
    ...
    learn: {
      ...
      links: [
        ...
        { page: 'grow', label_en: 'Learn by growing something', label_es: 'Aprende cultivando algo' }
      ],
  ...
  function getCacheVersion() { return 'v30'; }
